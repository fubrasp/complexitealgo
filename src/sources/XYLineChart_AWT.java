 ...
 * 
 * @author ARIF_BERTRAND_BOUGUETTOUCHA_GADEAU_SANCHO il s'agit d'une librairie:
 *         jfreechart (la documentation est payante!)
 */
public class XYLineChart_AWT extends ApplicationFrame {
	// pour enlever la legende on passer un booleen dans le constructeur sur
	// demande qui se met au bon etat en fonction du nb de segments
	public XYLineChart_AWT(String applicationTitle, String chartTitle, ArrayList<sources.Segment> arrSegs) {
		super(applicationTitle);
		// on creer un graphe en renseignant les axes, l'orientation, en y
		// incorporant les donnees, plusieurs options sont disponible avec les
		// booleens
		JFreeChart xylineChart = ChartFactory.createXYLineChart(chartTitle, "X", "Y", createDataset(arrSegs),
	 ...
	 * 
	 * @param arrSegs
	 *            structure permettant de representer le graphe
	 * @return collection que l'on peut parcourir aisement et ainsi representer
	 *         le graphe
	 */
	private XYDataset createDataset(ArrayList<sources.Segment> arrSegs) {
		...
		for (int x = 0; x < arrSegs.size(); x++) {
			...
			// on ajoute a la structure encapsulant toutes les XYSeries (ici les
			// segments!)
			xySeriesCollection.addSeries(test);
		}
		// on retourne la bonne structure de donnees contenant toutes les
		// informations utiles pour le trace
